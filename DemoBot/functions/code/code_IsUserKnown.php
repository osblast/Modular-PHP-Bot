<?php
	...
	else
	{
		...
		for($tzIter=0; $tzIter<count($mem_instance_arr); $tzIter++)
		{
			...
			$prefix_o = ""; $prefix_v = ""; $unknown_flags = "";
			...
			if($chan_member->UnknownFlags != "")
				$unknown_flags = $chan_member->UnknownFlags;

			if($onchanlist == "")
			{
				$onchanlist = "[$chan_member->attachedChannel/$prefix_o$prefix_v/$unknown_flags] ".$chan_member->Nickname;
			}
			else
			{
				$onchanlist = $onchanlist.", [$chan_member->attachedChannel/$prefix_o$prefix_v/$unknown_flags] ".$chan_member->Nickname;
			}
		}
