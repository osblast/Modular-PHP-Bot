...
if(count($wtt_data) < 5)
{
	$bot->socketHandler->send($bot->ircHandler->PRIVMSG("#phpbottest", "Please specify the channel name, Usage: WHATSTHETOPIC #Channel"));
}
else
{
	...
	{
		$bot->socketHandler->send($bot->ircHandler->PRIVMSG("#phpbottest", "Sorry, the channel $wtt_channel is not a valid channel name."));
		return;
	}
	...
	if($channelObj == null)
	{
		$bot->socketHandler->send($bot->ircHandler->PRIVMSG("#phpbottest", "Sorry, I'm not on the channel $wtt_channel so I can't help you there!"));
		return;
	}
